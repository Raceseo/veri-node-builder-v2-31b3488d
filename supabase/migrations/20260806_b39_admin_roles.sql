...
-- ────────────────────────────────────────────────────────────────────────────
-- §5. 기존 RLS 정책 재적용 (5개 / 테이블 3개)
--     submissions 1 + approval_workflow 2 + data_access_requests 2 = 5개.
--     user_roles 자신의 정책 2개는 Ray 결정 2에 따라 **의도적으로 만들지 않는다**(§2).
--
--     🔴 2026-08-06 실측 결과: 대상 테이블 3개가 **전부 서울에 없다**(to_regclass 전부 NULL).
--        → 이 절은 to_regclass 가드로 전부 건너뛴다. pg_policies 0행이 정상 동작이다.
--        → 승인 대상 테이블 신설은 B-39 범위 밖 — **출금 공사(B-31·B-32)로 이월**(Ray 판정).
--
