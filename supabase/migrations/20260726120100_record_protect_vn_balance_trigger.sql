-- 🔴 2026-08-19 실물 대조: 이 기록본은 실물과 불일치.
--    실물 = SECURITY DEFINER · search_path 미설정(당시) ·
--           판정 기준 (auth.jwt() ->> 'role') = 'authenticated'.
--    auth.jwt()는 DEFINER 영향을 받지 않으므로 가드는 정상 작동한다.
--    → 아래 19~20번 줄의 "INVOKER라야 한다 / DEFINER면 검사가 무의미"라는 근거는
--       이 구현엔 해당 없음(무효). 실물은 current_user가 아니라 auth.jwt()로
--       판정하므로 그 전제 자체가 성립하지 않는다.
--    → 실물 원문은 20260819_record_protect_vn_balance_ACTUAL.sql 참조.
-- ============================================================================
-- ============================================================================
