 ...
 *
 * 무엇이 문제였나:
 *   1. 마이데이터 연동이 **성공한 척한다.** handleMyDataConnect 는 setTimeout 2초 뒤
 *      isConnected=true 로 바꿀 뿐 DB 에 아무것도 쓰지 않는다. 새로고침하면 사라진다.
 *   2. 「서류 업로드」 「사진 촬영」 「직접 입력」 3버튼은 setSelectedMethod 만 호출한다.
 *      파일 선택창도 카메라도 입력 폼도 없다 — 파란 테두리만 생기고 끝난다.
 *   3. 「제출 방법별 신뢰 점수 +20/+12/+10/+5점」은 하드코딩된 표다.
 *      이 점수를 실제로 주는 코드는 어디에도 없다.
 *   4. 🔴 officialPartners 배열이 금융결제원·금융감독원·한국은행·KB국민은행·신한·
 *      하나·우리·카카오뱅크·토스뱅크를 「공식 마이데이터 파트너 / 정부 및 금융권
 *      인증 연동 기관」으로 표시한다. **실제 제휴가 없다.** 실재 기관과의 관계를
 *      사실로 표시하는 것이라 UI 문제가 아니라 법적 위험이다.
 *
 * 되살릴 때: 4번을 먼저 지운다. 그다음 1~3을 실제 동작으로 바꾼다.
 * 진입점만 되돌리면 위 네 가지가 그대로 살아난다.
 *
 * 백로그 B-86 · B-87. 차단 경위는 QuickMenu.tsx 상단 주석 참조.
 */
