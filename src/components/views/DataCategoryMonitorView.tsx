 ...
 *
 * dataCategories 는 DB 가 아니라 **상수 배열**이다. 연동이 0건인 신규 가입자에게도
 * 부동산 285만 + 소비 패턴 124만 + … 합계 **850만원**이 똑같이 표시된다(:182 합산 → 화면 상단).
 * updateLog 에는 "실시간 연동 중 - 데이터 신선도 최고" 라고 적혀 있다.
 *
 * 되살릴 때: 배열을 실데이터로 갈아끼우기 전에는 화면을 열지 않는다.
 * 값이 없으면 0 이나 빈 상태를 보여야 한다 — 그럴듯한 기본값을 채우지 않는다.
 *
 * 백로그 B-88. 차단 경위는 QuickMenu.tsx 상단 주석 참조.
 */
