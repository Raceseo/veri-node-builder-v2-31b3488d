/**
 * 🔴 온보딩 역할 선택 단계 제거로 **도달 불가** (2026-08-22).
 *    Index.tsx 의 'select-mode' 분기와 lazy import 를 함께 걷어냈다.
 *
 * 왜 없앴나: 「기업」을 고르면 실데이터에 연결되지 않은 더미 화면(Demand*, B-90)에
 *   착지했다. 신규 가입자가 첫인상으로 가짜를 보고, 새로고침하면 /enterprise 로 가
 *   다른 화면이 나오는 상태였다. 기업 문은 아직 열지 않았다(수요 사다리 ②단계).
 *
 * 🔴 기업 문을 여는 시점에 재검토할 것. 그때 **Auth.tsx 의 「가입 유형」 선택지와
 *    함께** 봐야 한다 — 원래 두 곳이 같은 것을 묻고 온보딩 쪽이 user_type 을
 *    덮어써서 뒤엣것이 이기는 구조였다. 한쪽만 되살리면 그 갈라짐이 돌아온다.
 */
import {
