...
- 기준 문서 위계 (위가 이긴다):
  ① VeriNode_구조정의서_v1 — 무엇을 하는 회사인가 (범위)
  ② 사업방향_v3 — 그 범위 안에서 무엇을 언제 하는가
  ③ 앱 정의서 v1.6 — 그 방향을 무엇으로 구현하는가
  이 절과 위 문서들이 어긋나면 문서가 이긴다.
  세 문서끼리 충돌하면 위 문서가 이긴다.
  전부 볼트: 03 Verinode/00사업계획서/
