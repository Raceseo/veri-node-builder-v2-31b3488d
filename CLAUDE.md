...
# 세션 마무리 3종 (종료 신호 시 확인)

① 배움노트  — 오늘 배운 것 (사건 + 다음엔 어떻게)
② 백로그    — 오늘 미룬 것
③ 인수인계서 — 오늘 한 것 + 다음 착수점

누락이 있으면 클박사에게 초안을 요청한다.
모두 볼트 파일 → 저장 = 완료, git 커밋 불필요.
단, CLAUDE.md는 저장소 파일이므로 이 변경은 커밋 대상.
채번은 반드시 원부 확인 후.
(파일 목록은 "저장된 것"만 안다. "쓰인 번호"는 참조처를 본다 — 백로그 「출처」 열 등)
